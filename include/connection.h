...
struct vega_io_commands {
    int (*try_read_command)(vega_conn *c); /* pointer for top level input parser */
...
struct vega_conn_ssl_attributes {
  SSL    *ssl;
  char   *ssl_wbuf;
  bool ssl_enabled;
};
...
struct vega_conn {
    ...
    struct vega_io_commands io_commands;
};
